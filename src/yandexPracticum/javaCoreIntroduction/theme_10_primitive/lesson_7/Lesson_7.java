...
public class Lesson_7 {
    public static void lesson_7() {
        printSection("Lesson_7. Финальный тренажер по типам");
    ...
    private static int makeRace(Car userCar, Car opponentCar, int distance) {
        ...
        if (shortRaceWin || longRaceWin) { // если победил на короткой или на длинной дистанции
            ...
            double max = Double.max(userCar.maxSpeed, opponentCar.maxSpeed);
            return (int) max;
    ...
    private static Car generateOpponentCar() {
        ...
        Integer nitroLevel = getNitroLevel(generateInt(1, 5));
    ...
    private static Car createCarByProperties(CarProperties carProperties) {
        ...
        int score = carProperties.initialScore; // Тут не явное преобразование типов
        Integer nitroLevel = Integer.valueOf(carProperties.nitroLevel);
